...
class two_layer_MLP:
    def __init__(self, input_size, size1, size2, output_size):
        ...
        self.b1 = np.zeros((size1, 1))
        self.b2 = np.zeros((size2, 1))
    ...
    def fit(self, X, y, batch_size, n_epochs, learning_rate=1e-8):
        ...
        for i in range(n_epochs):
            for k in range(0, N, batch_size):
                ...
                if k % 1000 == 0 and k != 0:
                    current_score = float(100 * score / (k + i * N))
                    print(
...
if __name__ == "__main__":
    X = np.load("/home/arthur/data/MNIST/digits.npy")
    X = X.astype(np.float32)
    y = np.load("/home/arthur/data/MNIST/target.npy")
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.2, shuffle=True, stratify=y, random_state=420
    )
    model = two_layer_MLP(X.shape[1], 800, 500, 10)
    start = time()
    model.fit(X_train, y_train, batch_size=10, n_epochs=10, learning_rate=1e-5)
